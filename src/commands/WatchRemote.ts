...
export class WatchRemote extends Watch {
    ...
    protected override async startNodemon(baseDir: string, scriptName: string): Promise<void> {
        ...
        await this.profileDir.spawn('npx', ['nodemon', '--config', 'nodemon.json', script], (exitCode: number) => {
